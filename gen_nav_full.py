...
html_file = os.path.join(current_dir, "fullprint.html")
...
print(f"Found {len(sections)} sections with id")
...
for section in sections:
    ...
    if not sec_id:
        continue
    heading = (
        section.find(["h1", "h2", "h3", "h4", "h5", "h6"])
        or section.find(attrs={"class": lambda c: c and "u-text" in c})
        or section.find("title")
    )

    if heading is None:
        print(f"Skipping {sec_id}: no heading found")
        print(section.prettify()[:1000])
        continue

    title = heading.get_text(" ", strip=True)
    print(f"Adding navigation item: {sec_id} -> {title}")
...
print(f"Navigation items created: {len(ul.find_all('li'))}")
...
print(len(soup.find_all("section", class_="u-clearfix u-section-16")))